...
T50501.r1 <- df %>% filter(scenario==50501) %>% group_by(cens) %>%
  summarise(m0.01.1 = mean(beta.a0.01.1),
            m0.01.2 = mean(beta.a0.01.2),
            m0.02.1 = mean(beta.a0.02.1),
            m0.02.2 = mean(beta.a0.02.2),
            m0.12.1 = mean(beta.a0.12.1),
            m0.12.2 = mean(beta.a0.12.2)) %>%
  select(-cens) %>% unlist

T50501.r2 <- df %>% filter(scenario==50501) %>% group_by(cens) %>%
  summarise(s0.01.1 = sd(beta.a0.01.1),
            s0.01.2 = sd(beta.a0.01.2),
            s0.02.1 = sd(beta.a0.02.1),
            s0.02.2 = sd(beta.a0.02.2),
            s0.12.1 = sd(beta.a0.12.1),
            s0.12.2 = sd(beta.a0.12.2)) %>%
  select(-cens) %>% unlist

T50501.r3 <- df %>% filter(scenario==50501) %>% group_by(cens) %>%
  summarise(s0.01.1 = mean(SE.beta.a0.01.1),
            s0.01.2 = mean(SE.beta.a0.01.2),
            s0.02.1 = mean(SE.beta.a0.02.1),
            s0.02.2 = mean(SE.beta.a0.02.2),
            s0.12.1 = mean(SE.beta.a0.12.1),
            s0.12.2 = mean(SE.beta.a0.12.2)) %>%
  select(-cens) %>% unlist

T50501.r4 <- df %>% filter(scenario==50501) %>% group_by(cens) %>%
  summarise(cp0.01.1 = mean(CoverCInorm(est = beta.a0.01.1, se = SE.beta.a0.01.1,
                                        truth = params$beta.a0.01[1])),
            cp0.01.2 = mean(CoverCInorm(est = beta.a0.01.2, se = SE.beta.a0.01.2,
                                        truth = params$beta.a0.01[2])),
            cp0.02.1 = mean(CoverCInorm(est = beta.a0.02.1, se = SE.beta.a0.02.1,
                                        truth = params$beta.a0.02[1])),
            cp0.02.2 = mean(CoverCInorm(est = beta.a0.02.2, se = SE.beta.a0.02.2,
                                        truth = params$beta.a0.02[2])),
            cp0.12.1 = mean(CoverCInorm(est = beta.a0.12.1, se = SE.beta.a0.12.1,
                                        truth = params$beta.a0.12[1])),
            cp0.12.2 = mean(CoverCInorm(est = beta.a0.12.2, se = SE.beta.a0.12.2,
                                        truth = params$beta.a0.12[2]))) %>%
  select(-cens) %>% unlist
